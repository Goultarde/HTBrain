...
func getModule(moduleUrl string, client *http.Client) (string, []string) {
	...
	// Append walkthrough solutions ONCE for the whole module
	if len(pageUrls) > 0 {
		firstPageContent := getModulePageContent(pageUrls[0], client)
		walkthroughContent, modErr := extractWalkthroughModal(firstPageContent)
		if modErr == nil && walkthroughContent != "" {
			walkthroughSection := "<hr />\n<h2>Walkthrough Solutions</h2>\n\n" + walkthroughContent
			pagesContent = append(pagesContent, walkthroughSection)
		}
	}
...
func getImagesLocally(htmlPages []string, moduleDir string) []string {
	var result []string
	for _, page := range htmlPages {
		...
		replaceImgs(doc, moduleDir)
		var htmlBuilder strings.Builder
		if err := html.Render(&htmlBuilder, doc); err != nil {
			die(err)
		}
		result = append(result, htmlBuilder.String())
	}
	return result
}

func replaceImgs(node *html.Node, moduleDir string) {
	if node.Type == html.ElementNode && node.Data == "img" {
		for i, attr := range node.Attr {
			if attr.Key == "src" {
				fileName := downloadImage(node.Attr[i].Val, moduleDir)
	...
	for child := node.FirstChild; child != nil; child = child.NextSibling {
		replaceImgs(child, moduleDir)
	}
}

func downloadImage(fileUrl string, moduleDir string) string {
	...
	// Créer le chemin complet du fichier dans le dossier du module
